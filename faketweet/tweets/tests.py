...
class TweetTestCase(TestCase):
    ...
    def test_action_like(self):
        client = self.get_client()
        response = client.post("/api/tweets/action/", {"id":1, "action":"like"}) #데이터 얻기 위해 post #like +1되는지 확인
        self.assertEqual(response.status_code, 200)
    ...
    def test_action_unlike(self):
        client = self.get_client()
        response = client.post("/api/tweets/action/", {"id":2, "action":"like"}) #데이터 얻기 위해 post #like +1되는지 확인
        self.assertEqual(response.status_code, 200)
        response = client.post("/api/tweets/action/", {"id": 2, "action": "unlike"})  # 데이터 얻기 위해 post #like +1되는지 확인
        self.assertEqual(response.status_code, 200)
        like_count = response.json().get("likes")
    ...
    def test_action_retweet(self):
        ...
        response = client.post("/api/tweets/action/", {"id": 2, "action": "retweet"})  # 데이터 얻기 위해 post #like +1되는지 확인
